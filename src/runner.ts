...
async function runClaudeOnce(
  ...
  if (signal?.aborted) {
    return { rawStdout: "", stderr: "Aborted", exitCode: 130 };
  }
...
async function execClaude(name: string, prompt: string, signal?: AbortSignal): Promise<RunResult> {
  ...
  let exec = await runClaudeOnce(args, primaryConfig.model, primaryConfig.api, baseEnv, signal);
  ...
  if (primaryRateLimit && hasModelConfig(fallbackConfig) && !sameModelConfig(primaryConfig, fallbackConfig)) {
    ...
    exec = await runClaudeOnce(args, fallbackConfig.model, fallbackConfig.api, baseEnv, signal);
    usedFallback = true;
  }
...
export async function run(name: string, prompt: string, signal?: AbortSignal): Promise<RunResult> {
  return enqueue(() => execClaude(name, prompt, signal));
}
...
export async function runUserMessage(name: string, prompt: string, signal?: AbortSignal): Promise<RunResult> {
  return run(name, prefixUserMessageWithClock(prompt), signal);
}
