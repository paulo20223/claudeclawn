...
async function handleRoute(req: Request, url: URL, opts: StartWebUiOptions): Promise<Response> {
  ...
  if (url.pathname === "/api/skills" && req.method === "POST") {
    try {
      const body = await req.json();
      const result = await createSkill(body as { name?: unknown; description?: unknown; content?: unknown });
      if (opts.onSkillsChanged) await opts.onSkillsChanged();
      return json({ ok: true, ...result });
    } catch (err) {
      return json({ ok: false, error: String(err) });
    }
  ...
  if (url.pathname.startsWith("/api/skills/") && req.method === "DELETE") {
    try {
      const encodedName = url.pathname.slice("/api/skills/".length);
      const name = decodeURIComponent(encodedName);
