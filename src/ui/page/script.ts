export const pageScript = String.raw`    const $ = (id) => document.getElementById(id);
    ...
    const skillsViewEl = $("skills-view");
    const skillCreateFormEl = $("skill-create-form");
    const skillOpenCreate = $("skill-open-create");
    const skillBackList = $("skill-back-list");
    const skillsList = $("skills-list");
    const skillsStatusEl = $("skills-status");
    ...
    let lastRenderedSkills = [];
    let scrollAnimFrame = 0;
    let timezoneOffsetMinutes = 0;
    let activeTab = "jobs";
    let jobsPage = 0;
    const JOBS_PER_PAGE = 5;
    let skillsViewMode = "list";
    ...
    function clockFromSchedule(schedule) {
      ...
      const instant = new Date(shiftedNow.getTime() - timezoneOffsetMinutes * 60_000);
    ...
    function renderJobsList(jobs) {
      ...
      const now = new Date();
      const paginationEl = $("jobs-pagination");
      ...
      const totalPages = Math.ceil(withNext.length / JOBS_PER_PAGE);
      if (jobsPage >= totalPages) jobsPage = Math.max(0, totalPages - 1);
      const pageItems = withNext.slice(jobsPage * JOBS_PER_PAGE, (jobsPage + 1) * JOBS_PER_PAGE);

      quickJobsList.innerHTML = pageItems
        .map((j) => {
          ...
          const badges = [];
          if (j.recurring) {
            badges.push('<span class="job-badge job-badge-recurring">recurring</span>');
          } else {
            badges.push('<span class="job-badge job-badge-once">once</span>');
          }
          if (j.type === "script") {
            badges.push('<span class="job-badge job-badge-script">script</span>');
          }
          if (j.notify === false) {
            badges.push('<span class="job-badge job-badge-silent">silent</span>');
          } else if (j.notify === "error") {
            badges.push('<span class="job-badge job-badge-error-only">errors only</span>');
          }

          const promptRaw = String(j.prompt || "").trim();
          const promptPreview = promptRaw.slice(0, 100).replace(/\n/g, " ");
          const promptEllipsis = promptRaw.length > 100 ? "…" : "";
          ...
          '<div class="quick-job-item">' +
            '<div class="quick-job-item-main">' +
              ...
              (promptPreview ? '<div class="job-prompt-preview">' + esc(promptPreview + promptEllipsis) + '</div>' : '') +
              (expanded ? (
      ...
      if (paginationEl) {
        if (totalPages <= 1) {
          ...
          paginationEl.innerHTML =
            '<button class="pagination-btn" ' + (jobsPage <= 0 ? "disabled" : "") + ' data-jobs-page="' + (jobsPage - 1) + '">&larr;</button>' +
            '<span class="pagination-info">' + (jobsPage + 1) + ' / ' + totalPages + '</span>' +
            '<button class="pagination-btn" ' + (jobsPage >= totalPages - 1 ? "disabled" : "") + ' data-jobs-page="' + (jobsPage + 1) + '">&rarr;</button>';
        }
      }
    }
    ...
    async function refreshState() {
      try {
        ...
        lastRenderedSkills = Array.isArray(state.skills) ? state.skills : [];
        renderSkillsList(lastRenderedSkills);
        if (uptimeBubbleEl) {
    ...
    function switchTab(tab) {
      ...
      if (quickJobsView) quickJobsView.classList.add("quick-view-hidden");
      if (quickJobForm) quickJobForm.classList.add("quick-view-hidden");
      if (skillsViewEl) skillsViewEl.classList.add("quick-view-hidden");
      if (skillCreateFormEl) skillCreateFormEl.classList.add("quick-view-hidden");
      if (tab === "jobs") {
        if (quickView === "create") {
          if (quickJobForm) quickJobForm.classList.remove("quick-view-hidden");
        } else {
          if (quickJobsView) quickJobsView.classList.remove("quick-view-hidden");
        }
      } else if (tab === "skills") {
        if (skillsViewMode === "create") {
          if (skillCreateFormEl) skillCreateFormEl.classList.remove("quick-view-hidden");
        } else {
          if (skillsViewEl) skillsViewEl.classList.remove("quick-view-hidden");
        }
      }
    }
    ...
    function setSkillsView(view) {
      ...
      if (skillsViewEl) skillsViewEl.classList.toggle("quick-view-hidden", view !== "list");
      if (skillCreateFormEl) skillCreateFormEl.classList.toggle("quick-view-hidden", view !== "create");
    }
    ...
    function renderSkillsList(skills) {
      ...
      if (!items.length) {
        skillsList.innerHTML = '<div class="quick-jobs-empty">No skills yet. Create your first skill.</div>';
        return;
      }
        ...
        return (
          '<div class="quick-job-item">' +
            '<div class="quick-job-item-main">' +
              ...
              '<div class="skill-description">' + esc(s.description || "No description") + '</div>' +
            '</div>' +
            '<button class="quick-job-delete" type="button" data-delete-skill="' + escAttr(s.name || "") + '">Delete</button>' +
          '</div>'
        );
      }).join("");
    }
    ...
    function renderClockToggle() {
      ...
      clockToggle.className = "toggle-btn " + (use12Hour ? "on" : "off");
    ...
    document.addEventListener("click", function(event) {
      var target = event.target;
      if (!(target instanceof HTMLElement)) return;
      var btn = target.closest("[data-jobs-page]");
      if (!btn || !(btn instanceof HTMLElement)) return;
      var page = Number(btn.getAttribute("data-jobs-page"));
    ...
    if (skillOpenCreate) {
      skillOpenCreate.addEventListener("click", function() { setSkillsView("create"); });
    }
    if (skillBackList) {
      skillBackList.addEventListener("click", function() { setSkillsView("list"); });
    }

    document.addEventListener("click", async function(event) {
      var target = event.target;
      if (!(target instanceof HTMLElement)) return;
      var button = target.closest("[data-delete-skill]");
      if (!button || !(button instanceof HTMLButtonElement)) return;
      var name = button.getAttribute("data-delete-skill") || "";
      if (!name) return;
      button.disabled = true;
      if (skillsStatusEl) skillsStatusEl.textContent = "Deleting skill...";
      try {
        var res = await fetch("/api/skills/" + encodeURIComponent(name), { method: "DELETE" });
        var out = await res.json();
        if (!out.ok) throw new Error(out.error || "delete failed");
        if (skillsStatusEl) skillsStatusEl.textContent = "Deleted " + name;
        await refreshState();
      } catch (err) {
        if (skillsStatusEl) skillsStatusEl.textContent = "Failed: " + String(err instanceof Error ? err.message : err);
      } finally {
        button.disabled = false;
      }
    });

    if (skillCreateFormEl) {
      skillCreateFormEl.addEventListener("submit", async function(event) {
        event.preventDefault();
        var nameInput = $("skill-name-input");
        var descInput = $("skill-desc-input");
        var contentInput = $("skill-content-input");
        var status = $("skill-form-status");
        var submit = $("skill-submit");
        ...
        if (!name || !description) {
          if (status) status.textContent = "Name and description are required.";
          return;
        }

        if (submit) submit.disabled = true;
        if (status) status.textContent = "Creating skill...";
        ...
        try {
          ...
          var out = await res.json();
          if (!out.ok) throw new Error(out.error || "failed");
          if (status) status.textContent = "Created " + name;
          ...
          if (status) status.textContent = "Failed: " + String(err instanceof Error ? err.message : err);
        } finally {
