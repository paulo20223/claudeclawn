...
function parseJobFile(name: string, content: string): Job | null {
  ...
  if (!scheduleLine) {
    console.error(`Job "${name}": missing schedule field`);
    return null;
  }
  ...
  const formatRaw = formatLine
    ? parseFrontmatterValue(formatLine.replace("format:", "")).toLowerCase()
    : "";
  const format: "plain" | "html" | "markdown" =
    formatRaw === "html" ? "html"
    : formatRaw === "markdown" ? "markdown"
    : "plain";

  return { name, schedule, prompt, recurring, notify, type, format };
}
